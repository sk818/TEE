#!/usr/bin/env python3
"""
Download high-resolution RGB imagery for Bangalore from Google Earth Engine.
...
# Configuration
BBOX = (77.46, 12.83, 77.78, 13.14)  # (min_lon, min_lat, max_lon, max_lat)
...
def search_high_res_imagery():
    """Search for available high-resolution imagery over Bangalore."""

    # Define area of interest
    min_lon, min_lat, max_lon, max_lat = BBOX
    aoi = ee.Geometry.Rectangle([min_lon, min_lat, max_lon, max_lat])

    print(f"\nSearching for high-resolution imagery over Bangalore...")
    print(f"Area: {BBOX}")
...
def main():
    ...
    # Check if high-res RGB already exists
    if OUTPUT_FILE.exists():
        print(f"\n✓ High-resolution RGB already cached at: {OUTPUT_FILE}")
        size_mb = OUTPUT_FILE.stat().st_size / (1024*1024)
    ...
    # Search for imagery
    source_info = search_high_res_imagery()
    if not source_info:
        return
